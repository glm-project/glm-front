...
`CurrentOperateurLifecycle` possède l'unique version courante de `DesignationOperateur` et en dérive les vues. Il possède aussi le rafraîchissement du référentiel, parce qu'il possède déjà la réconciliation qui en applique le résultat : `refreshReferentiel()` pour l'attendre, `pushReferentielFreshness()` pour le pousser en arrière-plan. Tout déclencheur passe par ces deux méthodes, `AtelierCoordinator.synchronize()` compris; leur liste vit dans [Offline pupitre](../../../../../../documentation/offline-pupitre.md). `AtelierCoordinator` orchestre les commandes de gestes et leur capture. La fenêtre porte l'exclusion pendant une intention globale initiée; les signaux de disponibilité la reflètent. Les deux orchestrations partagent la même file d'acceptation locale, que la fermeture attend sans dépendre du coordinateur de gestes.
